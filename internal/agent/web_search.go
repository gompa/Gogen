...
func (e *Executor) WebSearch(ctx context.Context, query string, maxResults int) (string, error) {
	if !webFetchEnabled() {
		return "", fmt.Errorf("web_search is disabled (set GOGEN_WEB_FETCH=on or GOGEN_WEB_SEARCH=on to enable)")
	}
	...
	backend := strings.ToLower(strings.TrimSpace(os.Getenv("GOGEN_WEB_SEARCH_BACKEND")))
	if backend == "brave" {
		if key := strings.TrimSpace(os.Getenv("GOGEN_WEB_SEARCH_API_KEY")); key != "" {
			return searchBrave(ctx, query, maxResults, key)
		}
	}
